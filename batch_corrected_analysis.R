...
# define basepaths
pathToProfiles <- here('..', 'data', 'batch_corrected_counts')
...
readProfiles <- function(pathToProfiles) {
  ...
  for (i in seq_along(profiles_file_names)) {  # Step 2
    ...
    ## Attention: Only makes sense for  BEATLE-batch-corrected tables
...
all_conditions <- summary_table$condition[1:10]
all_conditions <- c(all_conditions[3], all_conditions[-3])
...
all_data <- all_data %>%
  mutate(profiles_subsampled_hd = map(counts_long, function(x) return(x %>%
                                                                                   ...
                                                                                   relocate(sampleID, Taxa) %>% 
                                                                                    group_by(sampleID)  %>% 
                                                                                   rename(relAb = count)))) #Here, code is different from before_BEATLEcorrected

# join with metadata 
all_data <- all_data %>% 
  mutate(profiles_long_with_meta = map2(counts_long, metadata, function(pro, met) {
...
all_data <- all_data %>%
  mutate(metadata = map(metadata, function(x) return(x %>% as.data.frame() %>% select(condition) ))) %>%
...
heatmap_col_label <- c( "Cortez (n = 27)", "Iwasawa (n = 39)", "Lin (n = 149)", "Liu (n = 90)", "Ni (n = 109)", "Wang (n = 263)")
p8_heatmap <- pheatmap(fcs_selected, cluster_rows = F, cluster_cols = F, annotation_col = heatmap_groups, annotation_colors = ann_colors, 
                       ...
                       angle_col = 45, labels_row = make_bold_names(fcs_selected, rownames, c("Veillonella", "Fusobacterium", "Eubacterium", "Akkermansia")), display_numbers = p_vals_selected_asterisk)
